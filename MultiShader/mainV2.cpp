...
int main( void )
{
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		mat4 ModelMatrix = translate(mat4(1.0), vec3(0.0, 0.0, 0.0));
	...
	// Create and compile our GLSL program from the shaders. Todo: load shader
	//programID2 = LoadShaders("phong.vertexshader", "phong.fragmentshader");
   ...
   // while (!glfwWindowShouldClose(window))
    //{
		...
		//move object to the world origin
		mat4 ModelMatrix2 = translate(mat4(1.0), vec3(0.5, 1.5, 0.5));
		ModelMatrix2 = rotate( ModelMatrix2, rotZ2, vec3(0.0, 0.0, 1.0) );
		ModelMatrix2 = scale( ModelMatrix2, vec3(0.001, 0.001, 0.001) );
		ModelMatrix2 = translate(ModelMatrix2, -vec3(model2.center[0], model2.center[1], model2.center[2]));		// compute vertex normal in camera space (transpose of inverse View * Model)
		// You can compute it in GLSL if your GPU support OpenGL 3.x
		mat4 NormalMatrix2 = transpose( inverse(ViewMatrix2 * ModelMatrix2) );
		NormalMatrix = transpose( inverse(ViewMatrix2 * ModelMatrix2) );
